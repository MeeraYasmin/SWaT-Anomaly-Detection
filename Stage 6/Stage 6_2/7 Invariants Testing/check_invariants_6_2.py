...
# ── Paths ─────────────────────────────────────────────────────────────────────
_BASE_DIR = r"c:\Users\itrust\Downloads\Telegram Desktop\SWaT\Anomaly_Detection\Project_Steps (Stage 6)\Project_Steps (Stage 6_2)"
INVARIANTS_XLSX = os.path.join(_BASE_DIR, "6_Invariants_Generation", "invariants_6_2.xlsx")
DATA_PATH = os.path.join(_BASE_DIR, "3_Graph_Generation", "stage_6_components_2.csv")

OUTPUT_TEST = os.path.join(_BASE_DIR, "7_Invariants_Testing", "test_results_6_2.csv")
OUTPUT_COUNT = os.path.join(_BASE_DIR, "7_Invariants_Testing", "anomaly_count_6_2.csv")
OUTPUT_WINDOWSIZE = os.path.join(_BASE_DIR, "7_Invariants_Testing", "minimum_windowsize_6_2.csv")
OUTPUT_STATEMENTS_TXT = os.path.join(_BASE_DIR, "7_Invariants_Testing", "anomaly_statements_6_2.txt")
OUTPUT_ANOMALIES = os.path.join(_BASE_DIR, "7_Invariants_Testing", "anomalies_6_2.csv")
OUTPUT_DATASET = os.path.join(_BASE_DIR, "7_Invariants_Testing", "test_dataset_6_2.xlsx")
...
def transform_comment(comment):
    ...
    # Example: "P602 RUN implies flow on FIT601" -> "P602 RUN and no flow on FIT601 implies anomaly"
...
def analyze_invariants():
    if not os.path.exists(INVARIANTS_XLSX):
        print(f"Error: {INVARIANTS_XLSX} not found. Run generate_invariants_6_2.py first.")
        return
    ...
    # Save test_dataset_6_2.xlsx (cleaned input data, no invariant columns)
    ...
    for idx, row in inv_df.iterrows():
        ...
        try:
            ...
            # stats for anomaly_count_6_2.csv
            ...
            # stats for minimum_windowsize_6_2.csv
            window_results.append({
                "invariant_number": inv_id,
                "minimum_windowsize": int(min_window)
            })
            
            # anomalies_6_2.csv row
    ...
    def safe_to_csv(df, path):
        try:
            ...
            new_path = path.replace(".csv", "_updated.csv")
    ...
    safe_to_csv(pd.DataFrame(count_results), OUTPUT_COUNT)
    safe_to_csv(pd.DataFrame(window_results, columns=["invariant_number", "minimum_windowsize"]), OUTPUT_WINDOWSIZE)
    safe_to_csv(pd.DataFrame(anomalies_data), OUTPUT_ANOMALIES)
    ...
    try:
        ...
        new_txt = OUTPUT_STATEMENTS_TXT.replace(".txt", "_updated.txt")
        with open(new_txt, "w") as f:
            f.write("\n".join(statements_txt))
        print(f" - Warning: {OUTPUT_STATEMENTS_TXT} was locked. Saved to {new_txt} instead.")
