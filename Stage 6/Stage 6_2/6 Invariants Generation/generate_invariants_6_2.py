...
# ── Import threshold functions from compute_thresholds_6_2.py ──────────────────
...
# Import needed functions from Stage 6_2 computation script
...
def get_fit_thresholds(tag):
    """
    Calculates Tmin1 and Tmax1 for a specific FIT tag.
    Uses compute_min1 / compute_max1 as per context.
    """
...
def get_type(tag):
    """Maps a physical tag (e.g., FIT-601) to its generic type (FIT)."""
    for t in ["FIT", "LIT", "P"]:
        if tag.startswith(t):
            return t
    return None
...
def generate_instance_invariants(templates_path: str, connections_path: str) -> list[dict]:
    ...
    Returns a list of dictionaries with columns for invariants_6_2.xlsx.
    """
    templates = set()
...
if __name__ == "__main__":
    print("Generating instance-specific invariants for Stage 6_2...\n")
    rows = generate_instance_invariants(TEMPLATES_PATH, CONNECTIONS_PATH)

    output_path = os.path.join(os.path.dirname(__file__), "invariants_6_2.xlsx")
    ...
    if not rows:
        ...
        try:
            ...
            output_path = output_path.replace("invariants_6_2.xlsx", "invariants_6_2_updated.xlsx")
            df_rows.to_excel(output_path, index=False)
            print(f"Warning: invariants_6_2.xlsx was locked. Generated {len(rows)} invariants in {output_path} instead.")
